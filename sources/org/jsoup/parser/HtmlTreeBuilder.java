...
class HtmlTreeBuilder extends TreeBuilder {
    ...
    public List<Node> parseFragment(String inputFragment, Element context, String baseUri, ParseErrorList errors) {
        ...
        if (context != null) {
            ...
            if (StringUtil.m543in(contextTag, "title", "textarea")) {
                this.tokeniser.transition(TokeniserState.Rcdata);
            } else {
                if (StringUtil.m543in(contextTag, "iframe", "noembed", "noframes", "style", "xmp")) {
    ...
    public void popStackToClose(String... elNames) {
        Iterator<Element> it = this.stack.descendingIterator();
        while (it.hasNext()) {
            if (StringUtil.m543in(it.next().nodeName(), elNames)) {
                it.remove();
                return;
            }
            it.remove();
    ...
    private void clearStackToContext(String... nodeNames) {
        ...
        while (it.hasNext()) {
            Element next = it.next();
            if (!StringUtil.m543in(next.nodeName(), nodeNames) && !next.nodeName().equals(AdActivity.HTML_PARAM)) {
                it.remove();
            } else {
                return;
            }
    ...
    private boolean inSpecificScope(String[] targetNames, String[] baseTypes, String[] extraTypes) {
        ...
        while (it.hasNext()) {
            ...
            if (StringUtil.m543in(elName, targetNames)) {
                return true;
            }
            if (StringUtil.m543in(elName, baseTypes)) {
                return false;
            }
            if (extraTypes != null && StringUtil.m543in(elName, extraTypes)) {
                return false;
            }
        }
    ...
    public boolean inSelectScope(String targetName) {
        ...
        while (it.hasNext()) {
            ...
            if (!StringUtil.m543in(elName, TagSearchSelectScope)) {
                return false;
            }
        }
        Validate.fail("Should not be reachable");
    ...
    public void generateImpliedEndTags(String excludeTag) {
        while (excludeTag != null && !currentElement().nodeName().equals(excludeTag) && StringUtil.m543in(currentElement().nodeName(), TagSearchEndTags)) {
            pop();
        }
    }
    ...
    public boolean isSpecial(Element el) {
        return StringUtil.m543in(el.nodeName(), TagSearchSpecial);
    }
